...
""" Resampling algorithms. @namespace algo.resample """
...
def resample_systematic(numpy.ndarray[dtype=numpy.float64_t, ndim=1] w, double u):
    """
    ...
    cdef numpy.ndarray[dtype = numpy.int_t, ndim = 1] index = numpy.zeros(w.shape[0], dtype=numpy.int)
...
def resample_systematic_python(w, u):
    """
        Computes the particle indices by systematic resampling.
        \param w array of weights
    """
