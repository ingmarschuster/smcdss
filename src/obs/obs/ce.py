...
def solve_ce(f, n=5e4, model=binary.LogisticBinary, lag=0.2, elite=0.2, job_server=None, verbose=True):
    ...
    for step in xrange(1, 200):
        ...
        # show progress bar
        utils.aux.progress(r, ' %02i, %03i, objective: %.1f, time %s' % (step, len(model.r), best_obj, utils.aux.time(time.time() - t)))
        ...
        if len(model.r) < bf:
            ...
            if verbose: utils.aux.progress(1.0, ' %02i, %03i, objective: %.1f, time %s' % (step + 1, len(model.r), best_obj, utils.aux.time(time.time() - t)))
            break
