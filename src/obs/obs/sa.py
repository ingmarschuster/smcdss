...
def solve_sa(f, n=numpy.inf, m=numpy.inf, verbose=True):
    ...
    while True:
        ...
        if verbose:
            if r - s >= 0.01:
                utils.aux.progress(r, 'ar: %.3f, objective: %.1f, time %s' % (a / float(k), best_obj, utils.aux.time(time.time() - t)))
                s = r

        if r > 1:
            if verbose: utils.aux.progress(1.0, ' objective: %.1f, time %s' % (best_obj, utils.aux.time(time.time() - t)))
            break
